    ...
    def local_update(self, 
        ...
        for ith_update in range(I):
            for X, y in self.data_loader: 
                ...
                H = self.model(X)
                y_hat = F.softmax(H, dim=1)
                local_loss = F.nll_loss(torch.log(y_hat), y)
                local_loss.backward()
                optimizer.step()
            
            
    def cmode_eval(self, cmodel):
        with torch.no_grad():
...
class Central_Server:
    ...
    def eval_train_val(self):
        ...
        for k in self.train_ids:
            tloss, tacc = self.node_list[k].cmode_eval(self.cmodel)
        ...
        for k in self.val_ids:
            vloss, vacc = self.node_list[k].cmode_eval(self.cmodel)
    ...
    def eval_test(self):
        ...
        for k in self.test_ids:
            tloss, tacc = self.node_list[k].cmode_eval(self.best_cmodel)
